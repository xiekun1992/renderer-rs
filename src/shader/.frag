...
vec4 draw_box(vec2 vUv) {
    ...
    if (vUv.x <= border_left
        ...
        return texture(ourTexture, vec2(0.8, texture_y));
    } else if (
        ...
        return texture(ourTexture, vec2(0.4, texture_y));
    } else if (
        vUv.y <= border_bottom
    ) {
        return texture(ourTexture, vec2(0.6, texture_y));
    } else if (vUv.y >= 1.0 - border_top) {
        return texture(ourTexture, vec2(0.2, texture_y));
    } else {
